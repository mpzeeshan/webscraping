...
def scrap_data(containers1,containers2): 
...
def result_check(soup):
    for result in soup.findAll('div',attrs={'class':'sg-col-20-of-24 sg-col-28-of-32 sg-col-16-of-20 sg-col sg-col-32-of-36 sg-col-8-of-12 sg-col-12-of-16 sg-col-24-of-28'}):
...
soup = get_content(headers,url)
result_check(soup)
page_nos=int(get_pageNos())
store_content()
...
scrap_data(s_containers,l_containers)  

for i in range(page_nos+1)[2::1]:
    soup = get_content(headers,'https://www.amazon.in/s?k={userInput}&page={page_no}'.format(userInput= userInput,page_no=i))
    s_containers, l_containers= [],[]
    store_content()
    scrap_data(s_containers,l_containers)
